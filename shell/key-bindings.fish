...
#----END INCLUDE
  ...
  function fzf-history-widget -d "Show command history"
    ...
    set -lx -- FZF_DEFAULT_OPTS (__fzf_defaults '' \
      ...
      '--bind=\'shift-delete:execute-silent(eval builtin history delete --exact --case-sensitive -- (string escape -n -- {+} | string replace -ra "^\d*\\\\\\t" ""))+reload(eval $FZF_DEFAULT_COMMAND)\'' \
    ...
    if string match -qvr -- '--preview[= ]' "$FZF_DEFAULT_OPTS"
      ...
      set -p -- FZF_DEFAULT_OPTS \
        '--bind="focus,resize:transform:if test \\"$FZF_COLUMNS\\" -gt 100 -a \\\\( \\"$FZF_SELECT_COUNT\\" -gt 0 -o (string length -- {}) -gt (math $FZF_COLUMNS - 4) -o (string collect -- {2..} | fish_indent | count) -gt 1 \\\\); echo show-preview; else echo hide-preview; end"' \
